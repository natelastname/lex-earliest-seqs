...
| `X000012` | every prime factor lies in `p_1, p_4, p_9, p_16, ...` (square prime indices) |
| `X000013` | every prime factor lies in `p_1, p_2, p_4, p_8, p_16, ...` (power-of-two prime indices) |
| `X000014` | every prime factor lies in `p_1, p_4, p_27, p_256, ...` (self-power prime indices) |
| `X000015` | full-return EW for target pair `(2,3)` |
| `X000016` | full-return EW for target pair `(2,5)` |
| `X000017` | full-return EW for target pair `(3,5)` |
...
`X000009`--`X000011` are the **prime-coordinate-restricted** stride family. For parameter `k`, only coordinates `p_1, p_{1+k}, p_{1+2k}, ...` exist. Every prime factor of every generated noninitial term must come from this retained subsequence. Thus for `k = 3`, the allowed primes begin `2, 7, 17, 29, ...`, so no term is divisible by `3` or `5`. Both family generators accept arbitrary positive `k`; only `k = 2, 3, 4` are registered.

`X000012`--`X000014` are more aggressively thinned **prime-index-coordinate** variants. Their retained prime indices are respectively `i^2` for `i >= 1`, `2^i` for `i >= 0`, and `i^i` for `i >= 1`. All noninitial prime factors must lie in the selected retained subsequence.

`X000015`--`X000017` are the **full-return** family. For a fixed target pair `(p,q)`, ordinary EW is unchanged except when the immediately previous term is target-free. In that state a candidate involving exactly one of `p` or `q` is ineligible: the next candidate involving the target pair must contain both `p` and `q`. A one-sided target candidate can become eligible later once its immediate predecessor touches the target pair. The generator accepts arbitrary distinct prime pairs; only `(2,3)`, `(2,5)`, and `(3,5)` are registered. This one-free-term definition uses definition version 2; caches created by the earlier two-free-term interpretation are intentionally incompatible.
