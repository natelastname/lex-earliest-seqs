...
`X000012`--`X000014` are the **full-return** family. For a fixed target pair `(p,q)`, ordinary EW is unchanged except after two consecutive target-free terms. In that state a candidate involving exactly one of `p` or `q` is ineligible: the next candidate involving the target pair must contain both `p` and `q`. Once such a return occurs, proper one-sided target terms are again allowed until another two-target-free gap occurs. The generator accepts arbitrary distinct prime pairs; only `(2,3)`, `(2,5)`, and `(3,5)` are registered.
